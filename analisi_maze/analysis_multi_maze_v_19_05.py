...
Adattato da analysis_v_15_05.py per training multi-maze M1+M2 (ratio 1:2, BLOCK_PATTERN=(1,2,2)).
Aggiunge plot per-maze separati: reward, crash, steps, spawn, loss.

USAGE:
    Copia in cartella risultati assieme a training_log.csv (+ test_results.csv opzionale):
        python analysis_multi_maze_v_19_05.py

OUTPUT (plots/):
    01_reward_curve_global.png       reward globale + medie per-maze
    02_reward_curve_per_maze.png     curve separate M1 vs M2 (subplot)
    03_crash_rate_per_maze.png       crash rolling per maze
    04_steps_per_maze.png            steps medi per maze (rolling)
    05_spawn_analysis_M1.png         spawn breakdown M1
    06_spawn_analysis_M2.png         spawn breakdown M2
    07_loss_curve.png                loss globale lin+log
    08_maze_timeline.png             block timeline (quale maze in che ep)
    09_test_M1.png / 10_test_M2.png / 11_test_M3.png   (se test_results.csv)
    summary_training.txt             summary per-maze + globale
"""
...
def plot_reward_global(df):
    ...
    ax.plot(ep, reward,  alpha=0.18, lw=0.6, color='gray', label='reward (raw)')
    ax.plot(ep, rolling, lw=2.0,    color='black', label=f'global avg{WINDOW}')
    ...
    for m in MAZES_TRAIN:
        ...
        ax.plot(sub_ep, sub_roll, lw=1.8, color=MAZE_COLORS[m], alpha=0.9,
...
def plot_reward_per_maze(df):
    ...
    for ax, m in zip(axes, MAZES_TRAIN):
        ...
        ax.plot(ep, reward,  alpha=0.25, lw=0.7, color=color, label=f'{MAZE_LABEL[m]} raw')
        ax.plot(ep, rolling, lw=2.0,    color=color, label=f'{MAZE_LABEL[m]} avg{WINDOW_MAZE}')
        ax.axhline(0, color='black', lw=0.5, ls='--', alpha=0.4)
        ax.set_ylabel('Reward')
...
def plot_crash_per_maze(df):
    ...
    ax.plot(df['ep_global'], roll_all, lw=1.6, color='black', alpha=0.7,
    ...
    for m in MAZES_TRAIN:
        ...
        ax.plot(sub['ep_global'], roll, lw=2.0, color=MAZE_COLORS[m], alpha=0.9,
                label=f'{MAZE_LABEL[m]} avg{WINDOW_MAZE}')
...
def plot_loss_curve(df):
    ...
    ax1.plot(ep, loss,    alpha=0.2, lw=0.7, color='mediumpurple', label='loss (raw)')
    ...
    if len(loss_pos) > 0:
        ax2.plot(ep_pos, loss_pos, alpha=0.2, lw=0.7, color='mediumpurple', label='loss (raw)')
...
def plot_maze_timeline(df):
    ...
    for m in MAZES_TRAIN:
        ...
        if sub_ep.empty:
            continue
        ax.scatter(sub_ep, [m] * len(sub_ep), s=4,
