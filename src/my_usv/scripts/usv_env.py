...
# Random spawn positions per maze
SPAWN_LISTS = {
    1: [
        (-2.9, -2.0, 1.571),  # P1: left channel, heading N
        ( 1.0, -1.0, 1.571),  # P2: inner chamber, heading N
    ],
    2: [
        (-6.0,  0.0,  0.0  ),  # A1: heading E
        (-4.5,  1.5,  2.356),  # B3: heading NW
        (-7.0,  5.0,  0.0  ),  # C2: heading E
        ( 1.5,  0.0,  3.142),  # D1: heading W
        ( 0.5, -2.0,  1.571),  # D2: heading N
        ( 3.5,  0.5,  4.712),  # D3: heading S
        ( 0.0,  3.5,  3.142),  # E2: heading W
        (-4.5, -3.5,  0.0  ),  # F1: heading E
        (-1.5, -4.0,  1.571),  # F2: heading N
        ( 6.0,  6.0,  3.142),  # F3: heading W
    ],
    3: [
        (-2.0, -1.0,  0.0  ),
    ],
...
class UsvEnv(Node):
    ...
    def reset_environment(self, maze_id: int = 1, test_mode: bool = False) -> np.ndarray:
        ...
        for attempt in range(SPAWN_MAX_RETRIES):
            ...
            self.accepting_scans = False
            self.get_logger().warn(f"Spawn unsafe retry {attempt+1}")
    ...
    def step_action(self, action_index: int):
        ...
        while not self.new_scan_received:
            ...
            if (self.get_clock().now() - t0).nanoseconds * 1e-9 > timeout:
                self.get_logger().warn("Timeout scan! Gazebo lagga.")
                break
    ...
    def get_state(self) -> np.ndarray:
        ...
        state = np.nan_to_num(state, nan=1.0, posinf=1.0)
        return np.clip(state, 0.0, 1.0).copy()
